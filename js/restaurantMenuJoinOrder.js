...
var classTypes = [];
var itemsSelected = [];
var totPrice = parseFloat(getCookie("currentTotal"),10);
var userTot = 0.0;
var itemsPrices = [];
var ItemNames = [];
          ...
          row.onclick = function (myrow) {
              return function () {
                  ...
                  if (document.getElementById(id).style.background.toString().indexOf("aquamarine") >= 0) {
                      ...
                      userTot -= parseFloat(myrow.cells[1].innerHTML.substring(1), 10);
                      updateTotal();
                      printDiff(totPrice,minimum);
                  }
                  else {
...
function printDiff(total, minimum){
  ...
  var str = ""
  if(remainingPer <100){
    str = "Complete: "+remainingPer.toFixed(0)+"%";
    document.getElementById("myTotal").style.width=Math.round(remainingPer)+"%";
  }
  else{
    console.log("100% oops");
    str = "Percent complete: 100%"
    document.getElementById("myTotal").style.width = "100%";
  }
  document.getElementById("myTotal").innerHTML= str;

  //document.getElementById("myTotal").style.width=Math.round(remainingPer)+"%";//('style',"width:"+);
  console.log("THe val: " + Math.round(remainingPer)+"%");
...
function updateTotalCookie(){
    setCookie("total", userTot.toFixed(2));
}

function updateTotal(){
    var elem = document.getElementById("total");
    elem.innerHTML = "Total: $"+userTot.toFixed(2);
